...
CFPPTPanel::~CFPPTPanel()
{
  ...
  SAFE_RELEASE(m_btnNextLink);
  SAFE_RELEASE(m_btnPrevLink);
  SAFE_RELEASE(m_btnActLink);
...
BOOL CFPPTPanel::Create()
{
  ...
  // next-link button
  CFBitmap *bmpNextLink= new CFBitmap(m_hdc);
  if (!bmpNextLink->Load(IDB_BUTTON_NEXT_HL)) return(FALSE);
  CFBitmap *bmpNextLinkPressed= new CFBitmap(m_hdc);
  ...
  // prev-link button  
  CFBitmap *bmpPrevLink= new CFBitmap(m_hdc);
  if (!bmpPrevLink->Load(IDB_BUTTON_PREV_HL)) return(FALSE);
  CFBitmap *bmpPrevLinkPressed= new CFBitmap(m_hdc);
  ...
  CFBitmap *bmpActLink= new CFBitmap(m_hdc);
  if (!bmpActLink->Load(IDB_BUTTON_HYPER)) return(FALSE);
  CFBitmap *bmpActLinkPressed= new CFBitmap(m_hdc);
