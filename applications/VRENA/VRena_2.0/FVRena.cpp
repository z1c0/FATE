...
void CFVRena::Draw()
{
  ...
  if (m_bFirst) {
    ...
    if (!FillItemListsFromServer()) {
      Error(TEXT("Connection to File-Server failed!"));
    }

    // add manual entry
    m_aItemLists[1]->AddItem(TEXT("GRABBER"), TEXT("GRABBER"));
  }
}
        ...
        switch(iState) {
          ...
          case 2:
            if ((cRead[0] == '\n')||(cRead[0] == EOF)) {  // IP-settings complete
              iState= 0;
              if (!_stricmp(szHostName, "PPT1")) {                
                // first PPT host is automatically the file-server for MEDIA files
                CFInetAddr *pAddr= new CFInetAddr(szHostAddr, atoi(szPort)); 
                m_panelPPT[0]->SetAddrPPT(pAddr);
                m_addrFile= pAddr;

              } else if (!_stricmp(szHostName, "PPT2")) {
                m_panelPPT[1]->SetAddrPPT(new CFInetAddr(szHostAddr, atoi(szPort)));
              
              } else if (!_stricmp(szHostName, "PPT3")) {
                m_panelPPT[2]->SetAddrPPT(new CFInetAddr(szHostAddr, atoi(szPort)));
              
              } else if (!_stricmp(szHostName, "Video1")) {
                m_addrVideo[0]= new CFInetAddr(szHostAddr, atoi(szPort));
              
              } else if (!_stricmp(szHostName, "Video2")) {
                m_addrVideo[1]= new CFInetAddr(szHostAddr, atoi(szPort));
              
              } else if (!_stricmp(szHostName, "Video3")) {
...
//--------------------------------------------------------------------------------
BOOL CFVRena::SendToMovieHost(int iScreen, char *pMsg)
{
  CFSocket sock;
  ...
  // prepare message
  pBuff= (char*)malloc(iLen + 4);
  memcpy(pBuff, &iLen, 4);
  memcpy(&pBuff[4], pMsg, iLen);
