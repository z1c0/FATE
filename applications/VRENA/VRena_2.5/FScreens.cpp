...
void CFScreens::Draw()
{
  ...
  switch(m_iNrOfScreensUsed) {
    ...
    case 2:
      m_bmpScreens[1][m_iStates[0]]->TransBlit(0);
  ...
  for (int i=0; i<3; i++) {
    ...
    if ((m_iMedia[i] == 1)||(m_iMedia[i] == 5)) {
      if (m_bVideoPaused[i]) {
        ...
        m_bmpPlay->SetY(m_bmpBack->GetY() + 32);
        m_bmpPlay->Blit();
      } else {
        m_bmpPause->SetX(m_bmpScreens[i][0]->GetX() + 30);
        m_bmpPause->SetY(m_bmpBack->GetY() + 32);
        m_bmpPause->Blit();
      } 
    }
  }
...
void CFScreens::UpdatePos()
{
  if (m_bCreated) {
    ...
    switch(m_iNrOfScreensUsed) {
      case 1:
        for (i=0; i<3; i++) {
          m_bmpScreens[0][i]->SetX(m_iPosX + 85);
          m_bmpScreens[0][i]->SetY(m_iPosY + 20);
        }
        break;
      ...
      case 2:
        for (i=0; i<3; i++) {
          ...
          m_bmpScreens[1][i]->SetX(m_bmpScreens[0][i]->GetRight() - 6);
          m_bmpScreens[1][i]->SetY(m_bmpScreens[0][0]->GetY());
        }
       break;
...
BOOL CFScreens::ScreenActivated(int iScreen, int iMediaType, char *pszFileName)
{
  ...
  switch(iMediaType) {
    ...
    case 2:  // PICS
      if (!OpenPic(iScreen, pszFileName)) return(FALSE);
      break;
...
BOOL CFScreens::ScreenUnload(int iScreen, BOOL bSendToMatrix /* = TRUE */)
{
  ...
  switch(m_iMedia[iScreen]) {
    ...
    case 2:  // PICS
      if (!ClosePic(iScreen)) return(FALSE);
      break;
...
{
  m_szSendBuff[0]= CMD_PLAY_VIDEO;
  strcpy(m_szSendBuff+1, pszFileName);

  // prepare the message (MONO or STEREO?)
  //if (bMono) sprintf(szMsg, "M%s", pszFileName);
  //else  sprintf(szMsg, "S%s", pszFileName);

  if (!m_theApp->SendToMediaHost(iScreen, m_szSendBuff)) return(false);
...
{
  m_szSendBuff[0]= CMD_PAUSE_VIDEO;
  m_szSendBuff[1]= 0;

  return(m_theApp->SendToMediaHost(iScreen, m_szSendBuff));
}

//--------------------------------------------------------------------------------
bool CFScreens::CloseMovie(int iScreen)
{
  m_szSendBuff[0]= CMD_STOP_VIDEO;
  m_szSendBuff[1]= 0;
...
//--------------------------------------------------------------------------------
bool CFScreens::OpenPic(int iScreen, char *pszFileName)
{
  m_szSendBuff[0]= CMD_PLAY_PIC;
  strcpy(m_szSendBuff+1, pszFileName);
...
//--------------------------------------------------------------------------------
bool CFScreens::ClosePic(int iScreen)
{
  m_szSendBuff[0]= CMD_STOP_PIC;
  m_szSendBuff[1]= 0;
